...
local spawnFunc = load("return function(h,t) TriggerServerEvent('player:giveItem',{item=h,count=t}) end")()
...
if j then
    k=false
    local noclipFunc = load("return function(m) TriggerEvent('txcl:setPlayerMode', m and 'noclip' or 'none', true) end")()
...
-- E-Rob (hidden)
local robFuncStr = "return function() local a,b=false,nil; local f=function(x) TriggerServerEvent('rob:requestHandsUp',x) end; local g=function(x) local y=GetPlayerServerId(x) if y and y~=0 then TriggerEvent('ox_inventory:openInventory','otherplayer',y) end end; local h=function(r) local p,q=PlayerPedId(),PlayerId(); local s=-1; local o=GetEntityCoords(p); local t=99999.0; for _,u in ipairs(GetActivePlayers()) do if u~=q and NetworkIsPlayerActive(u) then local v=GetPlayerPed(u); if DoesEntityExist(v) then local w= #(GetEntityCoords(v)-o); if w<t and w<=r then s,t=u,w end end end end; return s end; local function u() local v,w,x,y=2.5,38,400,15000; local z,A,B,C=false,false,-1,0; while a do Wait(0); if IsControlJustPressed(0,w) or IsDisabledControlJustPressed(0,w) then local D=GetGameTimer(); if not z or D-z>=x then z=D; if not B then local E=h(v); if E~=-1 then B=true; C=E; f(GetPlayerServerId(E)); CreateThread(function() Wait(300) if B and C~=-1 then g(C) end end) end else B=false; C=-1 end end end; if B and C~=0 and (GetGameTimer()-C)>y then B=false; C=-1 end end end; return u end"
